...
{
    struct device_extension *ext = (struct device_extension *)device->DeviceExtension;
    DWORD len = wcslen(ext->instance_id);
    WCHAR *dst;

    if ((dst = ExAllocatePool(PagedPool, len * sizeof(WCHAR))))
        wcscpy(dst, ext->instance_id);
...
static WCHAR *get_compatible_ids(DEVICE_OBJECT *device, DWORD type)
{
    ...
    if ((dst = ExAllocatePool(PagedPool, len * sizeof(WCHAR))))
    {
        pos += swprintf(dst + pos, len - pos, L"%s\\%s", ext->device_id, ext->instance_id);
        dst[pos++] = 0;
        ...
        if (compatible_len)
        {
            pos += swprintf(dst + pos, len - pos, L"%s", ext->compatible_id);
            dst[pos++] = 0;
        }
    }
                                            ...
                                            struct unix_device *unix_device)
{
    ...
    swprintf(dev_name, ARRAY_SIZE(dev_name), L"\\Device\\%s#%p", bus_id, pnp_dev);
    RtlInitUnicodeString(&nameW, dev_name);
    status = IoCreateDevice(driver_obj, sizeof(struct device_extension), &nameW, 0, 0, FALSE, &device);
    if (status)
    {
        FIXME("failed to create device error %x\n", status);
        RtlFreeHeap(GetProcessHeap(), 0, pnp_dev);
        return NULL;
    }
    ...
    length = swprintf(ext->device_id, MAX_PATH, L"%s\\vid_%04x&pid_%04x", bus_id, desc->vendor_id, desc->product_id);
    if (desc->interface != (WORD)-1) swprintf(ext->device_id + length, MAX_PATH - length, L"&MI_%02i", desc->interface);

    swprintf(ext->instance_id, MAX_PATH, L"%i&%s&%x&%i", desc->version, ext->serialnumber, desc->location_id, ext->index);

    if (desc->is_gamepad)
    {
        length = swprintf(ext->compatible_id, MAX_PATH, L"%s\\vid_%04x&pid_%04x", bus_id, 0x045e, 0x0202);
...
static BOOL deliver_next_report(struct device_extension *ext, IRP *irp)
{
    ...
    ULONG buffer_len = stack->Parameters.DeviceIoControl.OutputBufferLength;
    ...
    if (buffer_len < report->length)
    {
        ERR("invalid HID read size %u, expected >= %u\n", buffer_len, report->length);
    ...
    if (ext->last_report != report) RtlFreeHeap(GetProcessHeap(), 0, report);
    return TRUE;
}
...
static void process_hid_report(DEVICE_OBJECT *device, BYTE *report_buf, DWORD report_len)
{
    ...
    if (!(report = RtlAllocateHeap(GetProcessHeap(), 0, offsetof(struct hid_report, buffer[report_len]))))
        return;
    ...
    if (ext->last_report) RtlFreeHeap(GetProcessHeap(), 0, ext->last_report);
    ext->last_report = report;
...
static void sdl_bus_load_mappings(struct sdl_bus_options *options)
{
    ...
    while (!status && info && mappings)
    {
        ...
        if (count > capacity)
        {
            ...
            if (!(mappings = RtlReAllocateHeap(GetProcessHeap(), 0, mappings, capacity * sizeof(*mappings)))) break;
        }
...
static NTSTATUS sdl_driver_init(void)
{
    ...
    sdl_params.map_controllers = check_bus_option(L"Map Controllers", 1);
    if (!sdl_params.map_controllers) TRACE("SDL controller to XInput HID gamepad mapping disabled\n");
    sdl_bus_load_mappings(&sdl_params);
    ...
    if (!(bus_thread[i] = CreateThread(NULL, 0, bus_main_thread, &params, 0, NULL)))
    {
        ...
        sdl_bus_free_mappings(&sdl_params);
        bus_count--;
        return STATUS_UNSUCCESSFUL;
    }
...
static NTSTATUS udev_driver_init(void)
{
    ...
    udev_params.disable_hidraw = check_bus_option(L"DisableHidraw", 0);
    if (udev_params.disable_hidraw) TRACE("UDEV hidraw devices disabled in registry\n");
    udev_params.disable_input = check_bus_option(L"DisableInput", 0);
...
static NTSTATUS pdo_pnp_dispatch(DEVICE_OBJECT *device, IRP *irp)
{
    ...
    switch (irpsp->MinorFunction)
    {
        ...
        case IRP_MN_REMOVE_DEVICE:
        {
            ...
            RtlFreeHeap(GetProcessHeap(), 0, pnp_device);
...
static NTSTATUS hid_get_device_string(DEVICE_OBJECT *device, DWORD index, WCHAR *buffer, DWORD length)
{
    ...
    case HID_STRING_ID_IMANUFACTURER:
        if (wcslen(ext->manufacturer) >= length) return STATUS_BUFFER_TOO_SMALL;
        else wcscpy(buffer, ext->manufacturer);
        return STATUS_SUCCESS;
    case HID_STRING_ID_IPRODUCT:
        if (wcslen(ext->product) >= length) return STATUS_BUFFER_TOO_SMALL;
        else wcscpy(buffer, ext->product);
        return STATUS_SUCCESS;
    case HID_STRING_ID_ISERIALNUMBER:
        if (wcslen(ext->serialnumber) >= length) return STATUS_BUFFER_TOO_SMALL;
        else wcscpy(buffer, ext->serialnumber);
        return STATUS_SUCCESS;
    }
...
static NTSTATUS WINAPI hid_internal_dispatch(DEVICE_OBJECT *device, IRP *irp)
{
    ...
    switch ((code = irpsp->Parameters.DeviceIoControl.IoControlCode))
    {
        ...
        case IOCTL_HID_GET_INPUT_REPORT:
        {
            ...
            if (packet->reportBufferLen < ext->last_report->length)
            {
                irp->IoStatus.Information = 0;
                irp->IoStatus.Status = STATUS_BUFFER_TOO_SMALL;
                break;
            ...
            memcpy(packet->reportBuffer, ext->last_report, ext->last_report->length);
            irp->IoStatus.Information = ext->last_report->length;
            irp->IoStatus.Status = STATUS_SUCCESS;
            break;
        }
        case IOCTL_HID_READ_REPORT:
            if (!deliver_next_report(ext, irp) && !set_pending_read(ext, irp))
            {
