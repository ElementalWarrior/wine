...
void wineXmlCallbackLog(char const* caller, xmlErrorLevel lvl, char const* msg, va_list ap)
{
    ...
    wine_dbg_log(dbcl, &__wine_dbch_msxml, NULL, NULL, caller, "%s", buff);
}
...
void wineXmlCallbackError(char const* caller, xmlErrorPtr err)
{
    ...
    wine_dbg_log(dbcl, &__wine_dbch_msxml, NULL, NULL, caller, "error code %d", err->code);
    if (err->message)
        wine_dbg_log(dbcl, &__wine_dbch_msxml, NULL, NULL, caller, ": %s", err->message);
    else
        wine_dbg_log(dbcl, &__wine_dbch_msxml, NULL, NULL, caller, "\n");
}
