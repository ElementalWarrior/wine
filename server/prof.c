...
static __attribute__((noinline)) void prof_initialize(void)
{
    struct timespec time_start, time_end;
    size_t warmup_count = 5000000, time_ns, ticks, tick_overhead;
    ...
    while (--warmup_count) {
        ...
        tick_overhead += prof_ticks();
        if (prof_overhead > tick_overhead / 2) prof_overhead = tick_overhead / 2;
    }
    ...
    ticks_per_ns = (ticks - prof_overhead + time_ns / 2) / time_ns;
    if (ticks_per_ns == 0) ticks_per_ns = 1;

    fprintf(stderr, "sprof:overhead %zu, ticks /ns: %zu\n", prof_overhead, ticks_per_ns);
...
size_t __cdecl __wine_prof_start( struct __wine_prof_data *data )
{
    ...
    if (__builtin_expect(trace_on == -1, 0) && !(trace_on = (getenv("WINEDEBUG") && strstr(getenv("WINEDEBUG"), "+sprof")))) return 0;
...
void __cdecl __wine_prof_stop( struct __wine_prof_data *data, size_t start_ticks )
{
    ...
    {
        fprintf(stderr, "sprof:%s: time spike: %zu (tcs) / %zu (tcs) %zu (%%)\n", data->name,
            spent_ticks, limit_ticks, spent_ticks * 100u / limit_ticks);
    }
                                    ...
                                    0, __ATOMIC_RELEASE, __ATOMIC_ACQUIRE))
    {
        if (accum_ticks * 100 >= spent_pctage * total_ticks)
            fprintf(stderr, "sprof:%s: time spent: %zu (tcs) / %zu (tcs) %zu (%%)\n", data->name,
                accum_ticks, total_ticks, accum_ticks * 100u / total_ticks);
